import { getPotentialTargets, getStrategy } from "./find-targets.js"; // added focusNode
import {
	getNetworkNodes,
	canPenetrate,
	getRootAccess,
...
export async function main(ns) {
	...
	async function getShips() {
		...
		var servers = nodes.filter(node => {
			...
			return hasRam(ns, node, virusRam) &&
				   (ns.hasRootAccess(node) || canPenetrate(ns, node, cracks))
	...
	function getDelayForActionSeq(seq, node) {
		...
		var wTime = 189.0; // times taken from the-hub
		var gTime = 60.0;
		var hTime = 162.0;
		var timing = {
			w: wTime,
	...
	while (true) {
		...
		var targets = getPotentialTargets(ns, priority, focusNode);
